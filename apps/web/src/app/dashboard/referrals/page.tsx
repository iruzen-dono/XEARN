...
import { Copy, Users, TrendingUp, CheckCircle, Award, ArrowDownRight, Share2, Crown } from 'lucide-react';
...
export default function ReferralsPage() {
  ...
  const [activeTab, setActiveTab] = useState<'level1' | 'level2' | 'level3' | 'commissions'>('level1');
  ...
  const tabs = [
    ...
    { key: 'level2' as const, label: `Niveau 2 (${level2.length})`, icon: TrendingUp, color: 'accent' },
    ...(l3Active ? [{ key: 'level3' as const, label: `Niveau 3 (${level3.length})`, icon: Crown, color: 'amber' }] : []),
  ...
  return (
    <div className="space-y-6">
      ...
      <MotionDiv preset="fadeUp" delay={0.1}>
        <div className="card-gradient">
          ...
          <div className="flex items-center gap-3">
            <input type="text" value={referralLink} readOnly className="input-field flex-1 text-sm font-mono" />
          ...
          <p className="text-dark-400 text-sm mt-3">
            Partagez ce lien. Quand quelqu&apos;un s&apos;inscrit, vous gagnez des commissions sur ses activités.
          </p>

          {/* Share buttons */}
          <div className="flex items-center gap-3 mt-4 pt-4 border-t border-white/[0.06]">
            <span className="text-dark-400 text-sm flex items-center gap-1.5"><Share2 className="w-4 h-4" /> Partager :</span>
            <button onClick={shareWhatsApp} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-[#25D366]/10 text-[#25D366] hover:bg-[#25D366]/20 transition-colors text-sm font-medium">
            ...
            <button onClick={shareTelegram} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-[#0088cc]/10 text-[#0088cc] hover:bg-[#0088cc]/20 transition-colors text-sm font-medium">
            ...
            <button onClick={shareFacebook} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-[#1877F2]/10 text-[#1877F2] hover:bg-[#1877F2]/20 transition-colors text-sm font-medium">
      ...
      <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {[
          { pct: l1Percent, label: 'Commission Niveau 1', desc: 'Sur les gains de vos filleuls directs', amount: stats?.commissionsL1, icon: <Users className="w-8 h-8" />, color: 'primary' },
          { pct: l2Percent, label: 'Commission Niveau 2', desc: 'Sur les gains des filleuls de vos filleuls', amount: stats?.commissionsL2, icon: <TrendingUp className="w-8 h-8" />, color: 'accent' },
          ...(l3Active ? [{ pct: l3Percent, label: 'Commission Niveau 3', desc: 'VIP exclusif — 3ème niveau de parrainage', amount: stats?.commissionsL3, icon: <Crown className="w-8 h-8" />, color: 'amber' }] : []),
        ].map((c, i) => (
          <motion.div key={i} variants={staggerItem}>
            <div className="card-hover text-center group">
              <div className={`p-3 rounded-2xl bg-gradient-to-br mx-auto w-fit mb-3 ${
                c.color === 'primary' ? 'from-primary-500/20 to-primary-500/5 text-primary-400' :
                c.color === 'accent' ? 'from-accent-500/20 to-accent-500/5 text-accent-400' :
                'from-amber-500/20 to-amber-500/5 text-amber-400'
              } group-hover:scale-110 transition-transform`}>
                {c.icon}
              </div>
              <div className="text-3xl font-bold gradient-text"><AnimatedCounter end={c.pct} suffix="%" /></div>
          ...
          <div className="card p-4 bg-gradient-to-r from-amber-500/5 to-yellow-500/5 border border-amber-500/20">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-dark-400 mt-0.5">Passez VIP pour gagner {l3Percent}% de commission sur un 3ème niveau de parrainage.</p>
              </div>
            </div>
          </div>
        </MotionDiv>
        ...
        <MotionDiv preset="fadeUp" delay={0.2}>
          <div className="card">
            <div className={`grid gap-4 text-center ${l3Active ? 'grid-cols-4' : 'grid-cols-3'}`}>
              <div>
                <div className="text-2xl font-bold text-primary-400"><AnimatedCounter end={stats.totalLevel1} /></div>
                <div className="text-dark-500 text-sm">Filleuls N1</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-accent-400"><AnimatedCounter end={stats.totalLevel2} /></div>
                <div className="text-dark-500 text-sm">Filleuls N2</div>
              </div>
              {l3Active && (
                <div>
                  <div className="text-2xl font-bold text-amber-400"><AnimatedCounter end={stats.totalLevel3 || 0} /></div>
                  <div className="text-dark-500 text-sm">Filleuls N3</div>
                </div>
              )}
              <div>
                <div className="text-2xl font-bold text-success-400">{fmt(stats.totalCommissions)}</div>
        ...
        <div className="card">
            ...
            <>
              ...
              {level1.length === 0 ? (
                <div className="empty-state">
                  ...
                  <p className="text-dark-400 text-sm max-w-xs">Partagez votre lien pour commencer !</p>
                </div>
              ) : (
                <div className="space-y-1">
                  {level1.map((ref: any, i: number) => (
                    <motion.div key={ref.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.03 }}
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="font-medium text-sm text-white">{ref.firstName} {ref.lastName}</div>
                          <div className="text-dark-500 text-xs">{new Date(ref.createdAt).toLocaleDateString('fr-FR')}</div>
                        </div>
                      </div>
                      <span className={`badge ${ref.status === 'ACTIVATED' ? 'badge-success' : 'badge-warning'}`}>
          ...
          {activeTab === 'level2' && (
            <>
              ...
              {level2.length === 0 ? (
                <div className="empty-state">
                  <div className="empty-state-icon"><TrendingUp className="w-8 h-8" /></div>
                  <h3 className="text-base font-semibold mb-1">Aucun filleul N2</h3>
                  <p className="text-dark-400 text-sm max-w-xs">Vos filleuls doivent aussi parrainer !</p>
                </div>
              ) : (
                <div className="space-y-1">
                  {level2.map((ref: any, i: number) => (
                    <motion.div key={ref.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.03 }}
                      className="flex items-center justify-between py-3 border-b border-white/[0.04] last:border-0">
                      <div className="flex items-center gap-3">
          ...
          {activeTab === 'level3' && l3Active && (
            <>
              ...
              {level3.length === 0 ? (
                <div className="empty-state">
                  <div className="empty-state-icon"><Crown className="w-8 h-8" /></div>
                  <h3 className="text-base font-semibold mb-1">Aucun filleul N3</h3>
                  <p className="text-dark-400 text-sm max-w-xs">Les filleuls de vos filleuls N2 apparaîtront ici.</p>
                </div>
              ) : (
                <div className="space-y-1">
                  {level3.map((ref: any, i: number) => (
                    <motion.div key={ref.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.03 }}
                      className="flex items-center justify-between py-3 border-b border-white/[0.04] last:border-0">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-gradient-to-br from-amber-500/20 to-amber-500/5 rounded-full flex items-center justify-center">
                          <span className="text-amber-400 font-semibold">{(ref.firstName || 'U')[0].toUpperCase()}</span>
                        </div>
                        <div>
                          <div className="font-medium text-sm text-white">{ref.firstName} {ref.lastName}</div>
                          <div className="text-dark-500 text-xs">{new Date(ref.createdAt).toLocaleDateString('fr-FR')}</div>
          ...
          {activeTab === 'commissions' && (
            <>
              ...
              {commissions.length === 0 ? (
                <div className="empty-state">
                  <div className="empty-state-icon"><Award className="w-8 h-8" /></div>
                  <h3 className="text-base font-semibold mb-1">Aucune commission</h3>
                  <p className="text-dark-400 text-sm max-w-xs">Les commissions arrivent quand vos filleuls complètent des tâches !</p>
                </div>
              ) : (
                <div className="space-y-1">
                  {commissions.map((c: any, i: number) => (
                    <motion.div key={c.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.03 }}
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        c.level === 1 ? 'bg-gradient-to-br from-primary-500/20 to-primary-500/5' :
                        c.level === 2 ? 'bg-gradient-to-br from-accent-500/20 to-accent-500/5' :
                        'bg-gradient-to-br from-amber-500/20 to-amber-500/5'
                      }`}>
                          <ArrowDownRight className={`w-5 h-5 ${
                            c.level === 1 ? 'text-primary-400' :
                            c.level === 2 ? 'text-accent-400' :
                            'text-amber-400'
                          }`} />
                        </div>
                        <div>
                          <div className="font-medium text-sm text-white">Commission N{c.level} — {c.sourceUser?.firstName} {c.sourceUser?.lastName}</div>
                          <div className="text-dark-500 text-xs">{new Date(c.createdAt).toLocaleDateString('fr-FR')} · {c.percentage}%</div>
                        </div>
                      </div>
