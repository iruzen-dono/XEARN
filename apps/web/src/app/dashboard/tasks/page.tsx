...
export default function TasksPage() {
  ...
  const handleOpenLink = useCallback(() => {
    ...
    if (url) { window.open(url, '_blank', 'noopener,noreferrer'); setLinkOpened(true); }
  }, [activeSession]);
  ...
  if (activeSession) {
    ...
    const progressPct = activeSession.minDurationSeconds > 0
      ? Math.min(100, ((activeSession.minDurationSeconds - countdown) / activeSession.minDurationSeconds) * 100)
      : 100;
    ...
    return (
      <div className="space-y-6">
        <MotionDiv preset="fadeUp">
          ...
          <p className="text-dark-400 mt-1">Complétez les étapes ci-dessous pour obtenir votre récompense</p>
        </MotionDiv>
        ...
        >
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-2xl bg-gradient-to-br ${TYPE_COLORS[activeSession.task.type] || 'from-primary-500/20 to-primary-500/5'}`}>
            ...
            <button onClick={handleCancel} className="p-2 text-dark-400 hover:text-white rounded-lg hover:bg-white/5 transition-colors">
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          {activeSession.task.description && (
            <p className="text-dark-300 mb-6 bg-white/[0.03] rounded-xl p-4 border border-white/[0.04]">{activeSession.task.description}</p>
          )}

          {/* Steps */}
          <div className="space-y-4 mb-8">
            {hasLink && (
              <div className={`flex items-center gap-4 p-4 rounded-xl border transition-all ${linkOpened ? 'border-success-500/30 bg-success-500/5' : 'border-white/[0.06] bg-white/[0.02]'}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${linkOpened ? 'bg-success-500 text-white' : 'bg-dark-700 text-dark-300'}`}>
                  {linkOpened ? '✓' : '1'}
                </div>
                <div className="flex-1">
                  <p className={`font-medium ${linkOpened ? 'text-success-400' : 'text-white'}`}>
                    {activeSession.task.type === 'VIDEO_AD' ? 'Regarder la vidéo' : activeSession.task.type === 'SURVEY' ? 'Compléter le sondage' : 'Visiter le lien'}
                  </p>
                  <p className="text-dark-500 text-sm">{linkOpened ? 'Lien ouvert ✓' : 'Cliquez pour ouvrir dans un nouvel onglet'}</p>
                </div>
                <button onClick={handleOpenLink} className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${linkOpened ? 'bg-success-500/10 text-success-400 border border-success-500/30' : 'btn-primary btn-sm'}`}>
                  <ExternalLink className="w-4 h-4" /> {linkOpened ? 'Ouvert' : 'Ouvrir'}
                </button>
              </div>
            )}

            <div className={`flex items-center gap-4 p-4 rounded-xl border transition-all ${countdown === 0 ? 'border-success-500/30 bg-success-500/5' : 'border-white/[0.06] bg-white/[0.02]'}`}>
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${countdown === 0 ? 'bg-success-500 text-white' : 'bg-dark-700 text-dark-300'}`}>
              ...
              <div className="flex-1">
                ...
                <p className="text-dark-500 text-sm">{countdown === 0 ? `Durée de ${activeSession.minDurationSeconds}s respectée ✓` : `Temps minimum : ${activeSession.minDurationSeconds} secondes`}</p>
              </div>
              {countdown > 0 ? (
          ...
          <div className="flex items-center justify-between pt-4 border-t border-white/[0.06]">
            ...
            <button onClick={handleComplete} disabled={!canComplete || completing} className="btn-primary py-3 px-8 flex items-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed text-base">
      ...
      {tasks.length === 0 ? (
        <MotionDiv preset="fadeUp" delay={0.1}>
          <div className="card text-center py-16">
            <div className="empty-state">
              ...
              <p className="text-dark-400 text-sm max-w-sm">De nouvelles tâches sont ajoutées régulièrement. Revenez bientôt !</p>
            </div>
          </div>
        </MotionDiv>
      ) : (
        <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="grid gap-3">
          {tasks.map((task) => {
            ...
            const typeColor = TYPE_COLORS[task.type] || 'from-primary-500/20 to-primary-500/5 text-primary-400';
            return (
              <motion.div key={task.id} variants={staggerItem}>
                <div className={`card-hover group flex flex-col sm:flex-row sm:items-center gap-4 ${isDone ? 'opacity-70' : ''}`}>
                  <div className="flex items-center gap-4 flex-1 min-w-0">
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-xl shrink-0 bg-gradient-to-br ${isDone ? 'from-success-500/20 to-success-500/5' : typeColor.split(' ').slice(0, 2).join(' ')}`}>
                      {isDone ? <CheckCircle className="w-6 h-6 text-success-400" /> : (TYPE_ICONS[task.type] || '📌')}
                    </div>
                    <div className="min-w-0">
                      <h3 className="font-semibold text-white truncate">{task.title}</h3>
                      <div className="flex items-center gap-2 mt-1">
                        <span className={`badge ${task.type === 'SPONSORED' ? 'badge-accent' : task.type === 'VIDEO_AD' ? 'badge-danger' : 'badge-primary'}`}>
                  ...
                  <div className="flex items-center gap-4 shrink-0">
                    ...
                    {isDone ? (
                      ...
                      <button onClick={() => handleStart(task.id)} disabled={starting === task.id} className="btn-primary btn-sm flex items-center gap-2">
                        {starting === task.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <Play className="w-4 h-4" />}
                        Commencer
                      </button>
                    )}
                  </div>
