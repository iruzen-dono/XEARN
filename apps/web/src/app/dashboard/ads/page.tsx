...
import { Plus, Loader2, Eye, Trash2, Clock, CheckCircle2, PauseCircle, XCircle, Megaphone } from 'lucide-react';
...
const STATUS_CONFIG: Record<string, { label: string; cls: string; icon: any }> = {
  ...
  PAUSED: { label: 'En pause', cls: 'bg-blue-500/10 text-blue-400 border-blue-500/20', icon: PauseCircle },
  EXPIRED: { label: 'Expirée', cls: 'bg-white/[0.04] text-dark-400 border-white/[0.06]', icon: Clock },
...
export default function AdsPage() {
  ...
  const [form, setForm] = useState({ title: '', description: '', mediaUrl: '', targetUrl: '', expiresAt: '' });

  const fetchAds = useCallback(async () => {
    if (!token) return;
    try {
      const data = await adsApi.getMine(token) as any;
      setAds(data?.ads || []);
    } catch (err) {
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      <MotionDiv preset="fadeUp">
        <div className="flex items-center justify-between">
          ...
          <motion.button whileTap={{ scale: 0.97 }} onClick={() => setShowCreate(!showCreate)} className="btn-primary flex items-center gap-2">
            ...
            <div className="card-hover">
              ...
              <form onSubmit={handleCreate} className="space-y-4">
                <div>
                  ...
                  <input type="text" className="input-field" placeholder="Titre de la publicité" minLength={3} maxLength={120}
                    value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} required />
                </div>
                <div>
                  <label className="text-sm text-dark-300 mb-1.5 block font-medium">Description</label>
                  <textarea className="input-field" rows={3} placeholder="Description optionnelle" maxLength={500}
                    value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} />
                </div>
                <div className="grid md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm text-dark-300 mb-1.5 block font-medium">URL média (image/vidéo)</label>
                  ...
                  <div>
                    <label className="text-sm text-dark-300 mb-1.5 block font-medium">URL cible (lien cliquable)</label>
                    <input type="url" className="input-field" placeholder="https://..."
                      value={form.targetUrl} onChange={(e) => setForm({ ...form, targetUrl: e.target.value })} />
                  </div>
                </div>
                <div>
                  <label className="text-sm text-dark-300 mb-1.5 block font-medium">Date d&apos;expiration</label>
                ...
                <div className="flex gap-3">
                  <motion.button whileTap={{ scale: 0.97 }} type="submit" disabled={creating}
                    ...
                    {creating ? <><Loader2 className="w-4 h-4 animate-spin" /> Création...</> : <><Plus className="w-4 h-4" /> Créer</>}
                  </motion.button>
        ...
        <MotionDiv preset="fadeUp" delay={0.1}>
          <div className="card">
            <div className="empty-state">
              <div className="empty-state-icon"><Megaphone className="w-8 h-8" /></div>
              <h3 className="text-base font-semibold mb-1">Aucune publicité</h3>
              <p className="text-dark-400 text-sm max-w-xs">Créez votre première publicité pour la diffuser auprès de nos utilisateurs.</p>
            </div>
          </div>
        </MotionDiv>
      ) : (
            ...
            return (
              <motion.div key={ad.id} variants={staggerItem}>
                <div className="card-hover flex flex-col md:flex-row md:items-center gap-4">
                  {ad.mediaUrl && (
                    <Image src={ad.mediaUrl} alt={ad.title} width={96} height={80} className="w-full md:w-24 h-20 object-cover rounded-xl bg-white/[0.02]" unoptimized />
                  )}
                  <div className="flex-1 min-w-0">
                    ...
                    {ad.description && <p className="text-dark-400 text-sm line-clamp-2">{ad.description}</p>}
                    <div className="text-dark-500 text-xs mt-1">
                      Créée le {new Date(ad.createdAt).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short', year: 'numeric' })}
                      {ad.expiresAt && <> · Expire le {new Date(ad.expiresAt).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short', year: 'numeric' })}</>}
                    </div>
                  </div>
                  <div className="flex items-center gap-1 shrink-0">
                    {ad.targetUrl && (
                      <a href={ad.targetUrl} target="_blank" rel="noopener noreferrer"
                        className="p-2.5 rounded-xl text-dark-400 hover:text-white hover:bg-white/[0.06] transition-colors" title="Voir le lien">
                        <Eye className="w-4 h-4" />
                      </a>
                    )}
                    <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}
                      onClick={() => handleDelete(ad.id)} disabled={deleting === ad.id}
                      className="p-2.5 rounded-xl text-dark-400 hover:text-danger-400 hover:bg-danger-500/10 transition-colors disabled:opacity-50" title="Supprimer">
