...
| **TasksModule** | CRUD tâches, complétion, crédit wallet, **filtrage par tier** (seules les tâches accessibles au niveau de l'utilisateur sont visibles) | PrismaModule, ReferralsModule |
| **WalletModule** | Portefeuille, transactions, activation, retraits avec **frais par tier** (10%/5%/2%), **upgrade de tier** | PrismaModule |
| **ReferralsModule** | Arbre parrainage **3 niveaux**, commissions (L1: 40%, L2: 10%, **L3 VIP: 5%**), stats | PrismaModule |
| **AdsModule** | CRUD publicités, **rôle Partenaire** (Pub Maker), **ciblage par pays et tier**, budget, approbation admin | PrismaModule |
| **PaymentModule** | Multi-provider (FedaPay recommandé, Mock), webhooks | PrismaModule |
| **NotificationsModule** | Notifications utilisateur (tâches, commissions, retraits), compteur non-lues | PrismaModule |
...
├── 2. Vérification: compte activé
├── 3. Vérification: **tier suffisant** (user.tier >= task.requiredTier)
├── 4. Vérification: pas déjà complétée par cet user
├── 5. Vérification anti-triche: session démarrée + temps minimum
...
├── 7. Distribuer commissions (async, try/catch):
...
└── 8. Réponse: complétion + wallet mis à jour
```
